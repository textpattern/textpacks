...
h3. Updating an exiting translation

After run sync tool, new trings will be empty.
You have to compare it with @en-gb.textpack@ in order to review and update it. It is burnsome!

The tip below will only help you translate new strings.
Note: Commit every thing before do it.

Firstly, filter all new strings:
...
Edit @xx-xx.txt@ in your favorite editor, then append it at the end of @xx-xx.textpack@:

bc. $ cat xx-xx.txt >>  xx-xx.textpack

Finally, sync then test.
